...
class StudentIDCardGenerator:
    ...
    def _create_background(self):
        ...
        for i in range(self.height):
            ratio = i / self.height
            r = int(self.colors['primary'][1:3], 16) * (1 - ratio) + int(self.colors['secondary'][1:3], 16) * ratio
            g = int(self.colors['primary'][3:5], 16) * (1 - ratio) + int(self.colors['secondary'][3:5], 16) * ratio
            b = int(self.colors['primary'][5:7], 16) * (1 - ratio) + int(self.colors['secondary'][5:7], 16) * ratio
            draw.line([(0, i), (self.width, i)], fill=(int(r), int(g), int(b)))
    ...
    def generate_id_card(self):
        ...
        # # --- Class + Section (center aligned below name area) ---
        # class_text = self._ensure_string(self.student.current_class) or ""
        # section_text = self._ensure_string(self.student.section) or ""

        # if class_text and section_text:
        #     combined_text = f"{class_text} - {section_text}"
        # else:
        #     combined_text = class_text or section_text  # fallback
        ...
        mobile = self._ensure_string(self.student.mobile)
        ...
        if father:
            ...
            guardian_name = self._ensure_string(father.name)
        elif mother:
            guardian_label = "Mother"
            guardian_name = self._ensure_string(mother.name)
        elif guardian:
            guardian_label = "Guardian"
            guardian_name = self._ensure_string(guardian.name)
        else:
            guardian_label = "Guardian"
            guardian_name = "N/A"
        ...
        # Father/Mother/Guardian at bottom but before blood group
        self._draw_info_line(draw, details_y + line_height * 5, 'id', guardian_label, guardian_name)

        if blood_group:
            self._draw_info_line(draw, details_y + line_height * 4, 'blood', 'Blood Group', blood_group)
    ...
    def _draw_placeholder_photo(self, draw, x, y, size):
        """Draw a placeholder photo circle"""
        draw.ellipse((x, y, x + size, y + size), fill='#E0E0E0')
        draw.text((x + size/2, y + size/2), "Photo", font=self.font_medium, 
